...
class DiffDrivePID(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('lookahead', 0.35)  # trailer hitch distance
        ...
        self.map_sub = self.create_subscription(OccupancyGrid,'/map', self.check_for_map, 10)
    ...
    def check_for_map(self,msg):
        self.map_check = True
        
    def goal_received(self, msg):
        """Callback for goal pose messages"""
        # Check if goal is in the correct frame (map)
        goal_frame = msg.header.frame_id
        
        new_goal = np.array([msg.pose.position.x, msg.pose.position.y])
        
        if np.linalg.norm(new_goal - self.desired_goal) < 0.01:
            return  # Goal same so skip
        
        if goal_frame != 'map' and goal_frame != '':
            self.get_logger().warn(f'Goal received in frame "{goal_frame}" but expecting "map". Attempting to transform...')
            try:
                # Try to transform the goal to map frame
                ...
                goal_x_map = cos_yaw * goal_x - sin_yaw * goal_y + tx
                goal_y_map = sin_yaw * goal_x + cos_yaw * goal_y + ty
                
                self.desired_goal = np.array([goal_x_map, goal_y_map])

                self.get_logger().info(f'Goal transformed to map frame: [{self.desired_goal[0]:.3f}, {self.desired_goal[1]:.3f}]')
            except Exception as e:
                self.get_logger().error(f'Failed to transform goal: {str(e)}')
                return
        else:
            # Goal is already in map frame (or no frame specified, assume map)
            self.desired_goal = np.array([msg.pose.position.x, msg.pose.position.y])

            self.get_logger().info(f'Goal set to: [{self.desired_goal[0]:.3f}, {self.desired_goal[1]:.3f}] in map frame')
            self.get_logger().info(f'Received goal in frame "{msg.header.frame_id}": '
                      f'({msg.pose.position.x:.3f}, {msg.pose.position.y:.3f})')
            self.get_logger().info(f'Set desired_goal to: ({self.desired_goal[0]:.3f}, {self.desired_goal[1]:.3f})')
    ...
    def publish_robot_cmd(self):
        ...
        try:
            ...
            trans = self._tf_buffer.lookup_transform(
                ...
                when, timeout=Duration(seconds=0.1))
        ...
        goal_dist = np.linalg.norm(self.desired_goal - self.robot_state[:2])
